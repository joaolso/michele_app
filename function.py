...
def train_and_SHAP(data_train):
    ...
    st.write('Avaliação do primeiro registro')
    ...
    st_shap(shap.force_plot(explainer.expected_value[0], shap_values[0], x), 400)


############################
    # df_train_normed_summary = shap.kmeans(X, 35)

    # # Instantiate an explainer with the model predictions and training data summary
    # explainer = shap.KernelExplainer(clf.predict, df_train_normed_summary)

    # # Extract Shapley values from the explainer
    # shap_values = explainer.shap_values(X)

    # # Summarize the Shapley values in a plot
    # st_shap(shap.force_plot(shap.summary_plot(shap_values, data_train.iloc[:,:34], max_display=100), x), 100)